<?php
    if($peticionAjax){
        require_once "../../modelo/admin/modelo_curso.php";
    }else{
        require_once "./modelo/admin/modelo_curso.php";
    }
    ...
    class controlador_curso extends modelo_curso{
        ...
        public function agregar_curso_controlador(){
            ...
            $check_curso=main_model::ejecutar_consulta_simple("SELECT * FROM curso WHERE TURNO_CUR='$turno' AND GRADO_CUR='$grado' AND SECCION_CUR='$seccion'");
            if($check_curso->rowCount()>0){
                $alerta=[
                    "Alerta"=>"simple",
                        "Titulo"=>"Ocurrio un error inesperado",
                        "Texto"=>"El CURSO ingresado ya se encuentra registrado en el sistema",
                        "Tipo"=>"error"
                ];
                echo json_encode($alerta);
                exit();
            ...
            if($total>=1 && $pagina<=$Npaginas){
                ...
                foreach($datos as $rows){
                    $tabla.='<tr class="text-center" >
                        ...
                        if($privilegio==1 || $privilegio ==2){
                            $tabla.='<td>
                                        <a href="'.SERVERURL.'admin/curso-update/'.main_model::encryption($rows['COD_CUR']).'/" class="btn btn-success">
                        ...
                        if($privilegio==1){
                            $tabla.='<td>
                                        <form class="FormularioAjax" action="'.SERVERURL.'ajax/admin/cursoAjax.php" method="POST" data-form="delete" autocomplete="off">
        ...
        public function actualizar_curso_controlador(){
            ...
            if($turno!=$campos['TURNO_CUR'] || $grado!=$campos['GRADO_CUR'] || $seccion!=$campos['SECCION_CUR']){
                $check_curso=main_model::ejecutar_consulta_simple("SELECT * FROM curso WHERE TURNO_CUR='$turno' AND GRADO_CUR='$grado' AND SECCION_CUR='$seccion'");
                if($check_curso->rowCount()>0){
                    $alerta=[
                        "Alerta"=>"simple",
