<?php
    if($peticionAjax){
        require_once "../modelo/modelo_padre.php";
    }else{
        require_once "./modelo/modelo_padre.php";
    }
    ...
    class controlador_padre extends modelo_padre{
        ...
        public function agregar_padre_controlador(){
            ...
            if(empty($_SESSION['datos_alumno'])){
                $alerta=[
                    "Alerta"=>"simple",
                    "Titulo"=>"Ocurrio un erro inesperado",
                    "Texto"=>"No has seleccionado ningun alumno",
                    "Tipo"=>"error"
                ];
                echo json_encode($alerta);
                exit();
        ...
        public function paginador_padre_controlador($pagina,$registros,$privilegio,$url,$busqueda,$ue){
            ...
            if(isset($busqueda) && $busqueda!=""){
                $consulta="SELECT SQL_CALC_FOUND_ROWS * FROM familiar AS F, alumno AS A, fa_alumno AS FA WHERE (F.NOMBRE_FA LIKE '%$busqueda%' 
                ...
                OR F.ROL_FA LIKE '%$busqueda%' OR F.CI_FA LIKE '%$busqueda%') AND FA.ALUMNO_ID=A.ALUMNO_ID AND FA.FAMILAR_ID=F.FAMILAR_ID AND A.UA_ID='$ue'
                ORDER BY F.NOMBRE_FA ASC LIMIT $inicio,$registros";
            }else{
                $consulta="SELECT SQL_CALC_FOUND_ROWS * FROM familiar AS F, alumno AS A, fa_alumno AS FA WHERE FA.ALUMNO_ID=A.ALUMNO_ID AND FA.FAMILAR_ID=F.FAMILAR_ID AND A.UA_ID='$ue' ORDER BY F.NOMBRE_FA ASC LIMIT $inicio,$registros";
            }
            ...
            if($total>=1 && $pagina<=$Npaginas){
                ...
                foreach($datos as $rows){
                    $tabla.='<tr class="text-center" >
                        ...
                        <td><button type="button" class="btn btn-info" data-toggle="popover" data-trigger="hover" title="'.$rows['NOMBRE_FA'].' '.$rows['APELLIDOP_FA'].' '.$rows['APELLIDOM_FA'].'" data-content="'.$rows['ROL_FA'].' de: '.$rows['NOMBRE_A'].' '.$rows['APELLIDOP_A'].' '.$rows['APELLIDOM_A'].'">
                        <i class="fas fa-info-circle"></i>
                        </button></td>';

                        if($privilegio==1 || $privilegio ==2){
                            $tabla.='<td>
                                        <a href="'.SERVERURL.'padre-update/'.main_model::encryption($rows['FAMILAR_ID']).'/" class="btn btn-success">
                        ...
                        if($privilegio==1){
                            $tabla.='<td>
                                        <form class="FormularioAjax" action="'.SERVERURL.'ajax/padreAjax.php" method="POST" data-form="delete" autocomplete="off">
        ...
        public function eliminar_padre_controlador(){
            ...
            $eliminar_padre=modelo_padre::eliminar_padre_modelo($id);
        ...
        public function buscar_alumno_controlador(){
            ...
            $id_alumno=main_model::decryption($_POST['buscar_alumno_up']);
            ...
            if($datos_alumno->rowCount()>=1){
                ...
                foreach($datos_alumno as $rows){
                    $tabla.='<tr class="text-center">
                                <td>'.$rows['CI_A'].' - '.$rows['NOMBRE_A'].' '.$rows['APELLIDOP_A'].' '.$rows['APELLIDOM_A'].'</td>
                                <td>';

                                    if($id_alumno!=""){
                                        $tabla.='<button type="button" class="btn btn-primary" onclick="agregar_alumno('.$rows['ALUMNO_ID'].','.$id_alumno.')"><i class="fas fa-tasks"></i></button>';
                                    }else{
                                        $tabla.='<button type="button" class="btn btn-primary" onclick="agregar_alumno('.$rows['ALUMNO_ID'].','."-1".')"><i class="fas fa-tasks"></i></button>';
                                    }
        ...
        public function agregar_alumno_controlador(){
            /**Recuperar id */
            $id=main_model::limpiar_cadena($_POST['id_agregar_alumno']);
            $id_padre=main_model::limpiar_cadena($_POST['id_agregar_alumno_up']);
            ...
            if($id_padre=="-1"){
                ...
                if(empty($_SESSION['datos_alumno'])){
                    ...
                    $alerta=[
                        ...
                        "Texto"=>"El Alumno se agrego para el padre",
                        "Tipo"=>"success"
                    ];
                    echo json_encode($alerta);
                }else{
                    $alerta=[
                        "Alerta"=>"simple",
                        "Titulo"=>"Ocurrio un error inesperado",
                        "Texto"=>"No hemos podido agregar el Alumno",
                        "Tipo"=>"error"
                    ];
                    echo json_encode($alerta);
                }
                ...
                $agregar_alumno=modelo_padre::agregar_alumno_padre_up_modelo($datos_alumno_up,$id_padre);

                if($agregar_alumno->rowCount()==1){
                    $alerta=[
                        "Alerta"=>"recargar",
                        "Titulo"=>"Alumno Agregado",
                        "Texto"=>"El Alumno se agrego",
                        "Tipo"=>"success"
                    ];
                }else{
                    $alerta=[
