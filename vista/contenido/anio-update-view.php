...
<div class="container-fluid">
    ...
    <?php
		...
		$datos_anio = $ins_anio->datos_anio_controlador("Unico",$pagina[1]);

		if($datos_anio->rowCount()==1){
			$campos = $datos_anio->fetch();
	?>

	<form class="form-neon FormularioAjax" action="<?php echo SERVERURL; ?>ajax/anioAjax.php" method="POST" data-form="update" autocomplete="off">
        <input type="hidden" name="anio_id_up" value="<?php echo $pagina[1]; ?>">
        <fieldset>
