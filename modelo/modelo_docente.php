<?php
    ...
    class modelo_docente extends main_model{
        protected static function agregar_docente_modelo($datos){
            $sql=main_model::conectar()->prepare("INSERT INTO profesor(UA_ID,CI_P,NOMBRE_P,APELLIDOP_P,APELLIDOM_P,FECHANAC_P,SEXO_P,FECHA_INGRESO_P,CORREO_P,CONTRA_P,TELEFONO_P,DIRECCION_P,ESTADO_P) 
            VALUES(:Ua,:CI,:Nombre,:ApellidoP,:ApellidoM,:FechaNac,:Sexo,:FechaIng,:Correo,:Contra,:Telefono,:Direccion,:Estado)");
        ...
        protected static function datos_docente_modelo($tipo,$id,$ue){
            if($tipo=="Unico"){
                $sql=main_model::conectar()->prepare("SELECT * FROM profesor WHERE PROFESOR_ID=:ID");
                $sql->bindParam(":ID",$id);
            }elseif($tipo=="Conteo"){
